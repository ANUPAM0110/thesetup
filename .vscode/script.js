...
function initComments() {
  ...
  if (!commentInput || !submitComment || !commentsContainer) return;
  ...
  submitComment.addEventListener('click', () => {
    ...
    commentsRef.push({
      ...
      commentInput.value = "";
      document.querySelector('.char-counter').textContent = '0/250';
    }).catch(error => {
  ...
  commentInput.addEventListener('input', function() {
    document.querySelector('.char-counter').textContent = `${this.value.length}/250`;
  });
...
document.getElementById("cart-close-btn").addEventListener("click", () => {
    ...
    // Enhanced showToast from Code 2
    function showToast(message, icon = "ℹ️") {
        const toastContainer = document.getElementById('toast-container') || createToastContainer();
        const toast = document.createElement('div');
        toast.className = 'toast';
  ...
  }, 300)); // Added debounce delay from Code 2
