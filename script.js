...
function setupCheckout() {
  ...
  function generateElectricParticles() {
  ...
  const size = Math.random() * 1.5 + 1; // Thin & varied
  particle.style.width = `${size}px`;
  particle.style.height = `${size * 20}px`;
...
// Generate particles repeatedly
setInterval(generateElectricParticles, 70);
    ...
    if (window.firebase && firebase.firestore) {
      ...
      db.collection('orders').doc(orderId).set(order)
        .then(() => {
          ...
          showToast('Order placed successfully! E-mailed the OrderId ', 'success');
